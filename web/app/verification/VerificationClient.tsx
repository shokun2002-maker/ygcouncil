    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px' }}>
        {[
          ...
          >
            <span style={{ fontSize: '0.8125rem', fontWeight: 800, color: st.done ? '#00A896' : st.active ? '#0066CC' : '#86868B' }}>
              STEP {st.step}
            </span>
      ...
      <div className="card-apple" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: '0.875rem', color: '#00A896', fontWeight: 700 }}>STEP 1 · 계정 연결</div>
          <div style={{ fontSize: '1.1875rem', fontWeight: 800, color: '#1D1D1F', marginTop: '2px' }}>카카오 간편 계정 연결</div>
          <div style={{ fontSize: '0.9375rem', color: '#6E6E73', marginTop: '4px' }}>{user.displayName} 계정으로 로그인되어 있습니다.</div>
        </div>
      ...
      <div className="card-apple" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: '0.875rem', color: isIdentityDone ? '#00A896' : '#0066CC', fontWeight: 700 }}>STEP 2 · 본인 식별</div>
            <div style={{ fontSize: '1.1875rem', fontWeight: 800, color: '#1D1D1F', marginTop: '2px' }}>휴대폰 본인확인 (PASS / NICE)</div>
          </div>
          {isIdentityDone ? (
            <span className="badge-apple" style={{ backgroundColor: '#D1FAE5', color: '#059669' }}>
              본인확인 완료
        ...
        {!isIdentityDone ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <button
              ...
              style={{ backgroundColor: '#0066CC', height: '48px', width: 'fit-content', padding: '0 24px', fontSize: '0.9375rem' }}
            >
              {isLoading ? '처리 중...' : '시연용 본인확인 진행'}
            </button>
          </div>
        ) : (
          <div style={{ fontSize: '0.9375rem', color: '#059669', fontWeight: 600 }}>
      ...
      <div className="card-apple" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: '0.875rem', color: isResidenceDone ? '#00A896' : isResidencePending ? '#0066CC' : '#6E6E73', fontWeight: 700 }}>STEP 3 · 거주지 증빙</div>
            <div style={{ fontSize: '1.1875rem', fontWeight: 800, color: '#1D1D1F', marginTop: '2px' }}>영광군 거주지 확인</div>
          </div>
          {isResidenceDone ? (
            <span className="badge-apple" style={{ backgroundColor: '#D1FAE5', color: '#059669' }}>
              거주지 승인 완료
        ...
        {!isIdentityDone ? (
          ...
          <div style={{ fontSize: '0.9375rem', color: '#1D1D1F', display: 'flex', flexDirection: 'column', gap: '6px' }}>
      ...
      >
        <div style={{ fontSize: '0.875rem', color: isResidenceDone ? '#00A896' : '#6E6E73', fontWeight: 700 }}>
          STEP 4 · 군민인증 결과
        </div>
        <h2 style={{ fontSize: '1.375rem', fontWeight: 800, color: '#1D1D1F' }}>
          {isResidenceDone ? '영광군민 인증 최종 완료' : '영광군민 인증 대기'}
        </h2>
        <p style={{ fontSize: '1rem', color: '#6E6E73', lineHeight: 1.6 }}>
          {isResidenceDone
        ...
        {isResidenceDone && (
          <div style={{ display: 'flex', gap: '16px', marginTop: '16px', flexWrap: 'wrap' }}>
            <Link href="/asks" className="btn-apple btn-apple-primary" style={{ backgroundColor: '#0066CC', height: '44px', padding: '0 24px' }}>
              의견수렴 참여하기
            </Link>
            <Link href="/listens/write" className="btn-apple btn-apple-secondary" style={{ height: '44px', padding: '0 24px' }}>
              내 이야기 들려주기
            </Link>
          </div>
        )}
