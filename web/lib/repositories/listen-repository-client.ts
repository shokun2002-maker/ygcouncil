...
export async function getProposalsClient(): Promise<Proposal[]> {
  try {
    ...
    console.warn('Proposals client query fallback used:', err);
    return MOCK_LISTEN_DATA;
  }
}

export async function getProposalByIdClient(id: string): Promise<Proposal | null> {
  const allProposals = await getProposalsClient();
